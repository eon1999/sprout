...
export default function RecommendationsPage() {
  ...
  const [loadingMsgIndex, setLoadingMsgIndex] = useState(Math.floor(Math.random() * loadingMessages.length));
  ...
  return (
    <main className="p-8 bg-lime-100 min-h-screen">
      <div className={`fixed inset-0 z-40 flex items-center justify-center bg-emerald-100 bg-opacity-30
        transition-opacity duration-500 ${loading ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
        ...
        shadow-lg transition duration-500 ease-in-out"
          onClick={handleRecommendations} disabled={loading}>
      ...
      {recommendations.length > 0 && (
        <div className="max-w-3xl mx-auto my-8 bg-green-200 p-4 rounded-lg shadow-md text-center
        transition-opacity duration-500">
          ...
          {recommendations.map((rec,index) => (
            <div key={index} className="p-2 bg-emerald-100 rounded-lg shadow-md text-center my-4">
